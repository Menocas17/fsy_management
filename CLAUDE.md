...
**Role permission summary:** any authenticated user can view; full edit access (companies, auxiliar companies, staffing) belongs to superadmin (`user.participant_id.nil?`), `coordinador`, and `director`; `auxiliar` can edit their own AC and their counselors' companies; `consejero` can edit only their own company; `logistica`/`registrador` are read-only.
