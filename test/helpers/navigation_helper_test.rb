...
class NavigationHelperTest < ActionView::TestCase
  test "lists the sections in sidebar order" do
    assert_equal [ "Inicio", "Jóvenes", "Staff", "Compañías", "Organigrama", "Logística", "Reportes", "Finanzas", "Mi perfil" ],
                 nav_items.map { |item| item[:text] }
  end

  test "shows Historial only to staff managers" do
  ...
  test "not-yet-built sections are disabled and have no destination" do
    ...
    assert_equal [ "Logística", "Reportes", "Finanzas" ], disabled.map { |item| item[:text] }
    assert disabled.all? { |item| item[:url].nil? }
  end

  test "groups items under the sidebar categories" do
    assert_equal [ nil, "Participantes", "Gestión", "Cuenta" ], nav_sections.map { |section| section[:label] }
    assert_equal [ "Compañías", "Organigrama", "Logística", "Reportes", "Finanzas" ],
